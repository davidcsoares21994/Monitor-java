...
try:
    import ollama
    OLLAMA_DISPONIVEL = True # Flag que verifica se a biblioteca 'ollama' está instalada
except ImportError:
...
# ==============================================================================
APP_VERSION = "18.0 (Captura de Evidências) - Análise Refinada"
ctk.set_appearance_mode("Dark")
ctk.set_default_color_theme("blue")
CORNER_RADIUS = 6
PADX_MAIN_CONTENT = 15
PADY_MAIN_CONTENT = 10
...
def analisar_bloco_com_ia(bloco_de_texto, ia_enabled=True):
    ...
    MODELO_OLLAMA_USADO = 'llama31-8b-f32:latest' # OU o nome exato que você usou no 'ollama create'
    ...
    **Log a ser analisado:**
    ...
    """
    try:
        response = ollama.chat(model=MODELO_OLLAMA_USADO, messages=[{'role': 'user', 'content': prompt}], format='json', stream=False)
...
class LogMonitorAppModern(ModernAppTemplate):
    ...
    def setup_monitor_page(self, parent_frame):
        """Cria a interface da página 'Monitor'."""
        parent_frame.grid_columnconfigure(0, weight=1)
        parent_frame.grid_rowconfigure(0, weight=0)
        parent_frame.grid_rowconfigure(1, weight=1)

        toolbar = self._create_toolbar(parent_frame, "Monitor de Processos")
        toolbar.grid(row=0, column=0, padx=PADX_MAIN_CONTENT, pady=(0, 10), sticky="ew")
        
        action_frame = ctk.CTkFrame(toolbar, fg_color="transparent")
        action_frame.pack(side="right", padx=5)

        self.start_button = ctk.CTkButton(action_frame, text="▶ Iniciar Análise", command=lambda: [self.log_action("Botão 'Iniciar Análise' clicado."), self.start_monitoring()], corner_radius=CORNER_RADIUS)
        self.start_button.pack(side=tk.LEFT, padx=(0, 5))
        self.stop_button = ctk.CTkButton(action_frame, text="■ Parar Análise", command=lambda: [self.log_action("Botão 'Parar Análise' clicado."), self.stop_monitoring()], state='disabled', corner_radius=CORNER_RADIUS, fg_color="#D32F2F", hover_color="#B71C1C")
        self.stop_button.pack(side=tk.LEFT, padx=(0, 5))
        self.export_button_monitor = ctk.CTkButton(action_frame, text="Exportar Log 📄", command=lambda: [self.log_action("Botão 'Exportar Log' (Monitor) clicado."), self.export_monitor_log()], corner_radius=CORNER_RADIUS)
        self.export_button_monitor.pack(side=tk.LEFT, padx=(5, 5))
        
        self.search_entry_monitor = ctk.CTkEntry(action_frame, textvariable=self.search_term_monitor, placeholder_text="Buscar no log...", corner_radius=CORNER_RADIUS, width=200)
        self.search_entry_monitor.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(5, 0))
        self.search_entry_monitor.bind("<Return>", lambda e: self.search_in_monitor())
        self.search_button_monitor = ctk.CTkButton(action_frame, text="🔎", command=self.search_in_monitor, width=40, corner_radius=CORNER_RADIUS)
        self.search_button_monitor.pack(side=tk.LEFT, padx=(5, 0))
    ...
    def setup_settings_page(self, parent_frame):
        ...
        ctk.CTkLabel(analysis_config_frame, text="Gatilhos de Análise (palavras-chave separadas por vírgula):").grid(row=2, column=0, padx=PADX_MAIN_CONTENT, pady=PADY_MAIN_CONTENT, sticky="w")
        self.entry_keywords = ctk.CTkEntry(analysis_config_frame, textvariable=self.keywords_var, corner_radius=CORNER_RADIUS)
        self.entry_keywords.grid(row=2, column=1, padx=PADX_MAIN_CONTENT, pady=PADY_MAIN_CONTENT, sticky="ew")

        ctk.CTkLabel(analysis_config_frame, text="Nº Máximo de Linhas no Log da Tela:").grid(row=3, column=0, padx=PADX_MAIN_CONTENT, pady=PADY_MAIN_CONTENT, sticky="w")
        self.entry_max_lines = ctk.CTkEntry(analysis_config_frame, textvariable=self.max_log_lines_var, corner_radius=CORNER_RADIUS)
        self.entry_max_lines.grid(row=3, column=1, padx=PADX_MAIN_CONTENT, pady=PADY_MAIN_CONTENT, sticky="ew")
        ...
        self.capture_evidence_checkbox.grid(row=4, column=0, columnspan=2, padx=PADX_MAIN_CONTENT, pady=PADY_MAIN_CONTENT, sticky="w")
    ...
    def setup_history_page(self, parent_frame):
        ...
        toolbar.grid(row=0, column=0, columnspan=2, padx=PADX_MAIN_CONTENT, pady=(0, 10), sticky="ew")
            ...
            while batch_size < 200:
                ...
                if msg_type == "raw_line":
                    ...
                    if msg_type in ["system_message", "system_error"]:
                        ...
                        threading.Thread(target=self.chamar_analise_avancada, args=(content,), daemon=True).start()
    ...
    def chamar_analise_avancada(self, bloco):
        if self.analysis_lock.acquire(blocking=False):
            try:
                ...
                resultado = analisar_bloco_com_ia(bloco, ia_enabled=self.ia_enabled_var.get())
    ...
    def set_ui_state(self, monitoring: bool):
        ...
        if hasattr(self, 'capture_evidence_checkbox'): self.capture_evidence_checkbox.configure(state=state)
    ...
    def escolher_executavel(self):
        ...
        workdir = os.path.dirname(filepath); self.workdir_var.set(workdir.replace("/", "\\"))
        if filepath.lower().endswith(".jar"): command = f'java -jar "{filepath}"'
        else: command = f'"{filepath}"'
    ...
    def load_selected_log(self):
        ...
        try:
            ...
            self.history_display.configure(state=tk.NORMAL); self.history_display.delete("1.0", tk.END); self.history_display.insert("1.0", content); self.history_display.configure(state=tk.DISABLED)
    ...
    def delete_selected_log(self):
        ...
        if messagebox.askyesno("Confirmar Exclusão", f"Tem certeza que deseja apagar permanentemente o arquivo '{filename}'?"):
            try:
                os.remove(os.path.join(HISTORY_DIR, filename)); self.populate_history_listbox()
                self.history_display.configure(state=tk.NORMAL); self.history_display.delete("1.0", tk.END); self.history_display.configure(state=tk.DISABLED)
    ...
    def stream_reader_worker(self, process, keywords_str):
    ...
    def start_monitoring(self):
        command_str = self.command_var.get(); workdir = self.workdir_var.get()
        if not command_str or not workdir: messagebox.showerror("Erro de Configuração", "Vá para a aba 'Configurações' e selecione a aplicação e o diretório de trabalho."); return
        if not os.path.isdir(workdir): messagebox.showerror("Erro", f"O diretório de trabalho é inválido:\n{workdir}"); return
        
        timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S"); self.current_session_filepath = os.path.join(HISTORY_DIR, f"sessao_{timestamp}.log")
        
        self.is_monitoring = True; self.limpar_log(); self.log_queue.put({"type": "system_message", "content": f"--- [ INICIANDO PROCESSO: {command_str} ] ---"})
        ...
        try:
            ...
            self.java_process = subprocess.Popen(args, cwd=workdir, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, encoding='latin-1', errors='replace', shell=False, creationflags=creation_flags_value)
            self.process_thread = threading.Thread(target=self.stream_reader_worker, args=(self.java_process, self.keywords_var.get()), daemon=True); self.process_thread.start()
            ...
            self.log_queue.put({"type": "system_error", "content": f"ERRO ao iniciar o processo: {e}"}); self.stop_monitoring()
    ...
    def generate_analysis_summary(self):
        """Gera e exibe um resumo consolidado das análises e conexões."""
        summary_messages_parts = []
        
        if self.analysis_results_summary_buffer:
            summary_messages_parts.append("\n" + "="*80 + "\n")
            summary_messages_parts.append("📊 RESUMO DA SESSÃO DE ANÁLISE 📊\n".upper())
            ...
            for category in ordered_categories:
                ...
                if items:
                    summary_messages_parts.append(f"\n--- {category} ({len(items)} ocorrências) ---\n")
                    for i, item in enumerate(list(set(items))[:5]): 
                        summary_messages_parts.append(f"  - {item}\n")
                    if len(set(items)) > 5:
                        summary_messages_parts.append("  ... e mais.\n")
        
        if self.detected_connections:
            summary_messages_parts.append("\n" + "="*80 + "\n")
            summary_messages_parts.append("🔗 CONEXÕES DE REDE DETECTADAS 🔗\n".upper())
            summary_messages_parts.append(f"\n({len(self.detected_connections)} endereços/URLs únicos):\n")
            for conn in sorted(list(self.detected_connections)): 
                summary_messages_parts.append(f"  - {conn}\n")
            summary_messages_parts.append("\n")
        
        if summary_messages_parts:
            summary_messages_parts.append(f"\n{'='*80}\n")
            self.log_queue.put({"type": "system_message", "content": "".join(summary_messages_parts)})
        else:
    ...
    def display_analysis_result(self, result_dict):
        ...
        self.log_textbox.see(tk.END); self.log_textbox.configure(state=tk.DISABLED)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if 'pyautogui' in libs_necessarias:
    ...
    if faltando:
        faltando_str = ', '.join(faltando)
        messagebox.showerror("Dependências Faltando", f"As seguintes bibliotecas são necessárias:\n\n{faltando_str}\n\nInstale com: pip install {faltando_str}")
        exit(1)
